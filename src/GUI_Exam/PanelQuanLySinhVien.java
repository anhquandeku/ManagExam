 ...
 * @author ASUS
 */
public class PanelQuanLySinhVien {
    ...
    public PanelQuanLySinhVien(String pnCenter) {
        ...
        info = temp;
        MyLabel lbMSV = new MyLabel("Mã sinh viên :");
        lbMSV.setPreferredSize(new Dimension(120, 30));
        top.add(lbMSV);
        ...
        MyLabel lbKhoa = new MyLabel(info[7]);
        ...
        MyLabel lbLop = new MyLabel(info[6]);
        LopBUS lop = new LopBUS();
        MyComboBox cbLop = new MyComboBox();
        cbLop.addItem("");
        cbKhoa.addItemListener(new ItemListener() {
            @Override
            public void itemStateChanged(ItemEvent e) {
                if (e.getSource() == cbKhoa) {
                    int itemCount = cbLop.getItemCount();
                    cbLop.removeAllItems();
                    cbLop.addItem("");
                    String temp = cbKhoa.getSelectedItem().toString();
                    ArrayList<String> arrLop = lop.getListLop(temp);
                   // lop.getListLop(temp).toArray(arrLop);
                    for (int i = 0; i < arrLop.size(); i++) {
                        cbLop.addItem(arrLop.get(i));
                    }
                }
            }
        });
        ...
        tba.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent arg0) {
                ...
                if (i != -1) {
                    ...
                    cbKhoa.setSelectedItem(String.valueOf(model.getValueAt(i, 7)));
                    cbLop.setSelectedItem(String.valueOf(model.getValueAt(i, 6)));
                }
            }
        });
        ...
        bts[1].addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent arg0) {
                if (txtHo.getText().equals("") || txtTen.getText().equals("") || cbNam.getSelectedItem().equals("YYYY") || cbThang.getSelectedItem().equals("MM") || cbNgay.getSelectedItem().equals("DD") || cbKhoa.getSelectedItem().equals("") || cbGioiTinh.getSelectedItem().equals("") || cbLop.getSelectedItem().equals("")) {
                    ...
                    err.btnOK.addMouseListener(new MouseAdapter() {
                        public void mouseClicked(MouseEvent arg0) {
                            ...
                            System.out.println(lopbus.getLopMaKhoa(maKhoa).size());
                            String maLop = lopbus.getLopMaKhoa(maKhoa).get(cbLop.getSelectedIndex()-1).getMaLop();
                            String ngaySinh = cbNam.getSelectedItem().toString() + "-" + cbThang.getSelectedItem().toString() + "-" + cbNgay.getSelectedItem().toString();
                            SinhVienDTO dtSV = new SinhVienDTO(maLop, maKhoa, txtHo.getText(), txtTen.getText(), ngaySinh, cbGioiTinh.getSelectedItem().toString(), 1);
                            sv.them(dtSV);
                            ...
                            cbKhoa.setSelectedItem("");
                            cbLop.setSelectedItem("");
                            mahoantac = String.valueOf(model.getValueAt(tba.getRowCount() - 1, 0));
                            check = 2;
                            guiBase.setEnabled(bts[3]);
                        }
                    });
                    ...
                    if (txtHo.getText().equals("") || txtTen.getText().equals("") || cbNam.getSelectedItem().equals("YYYY") || cbThang.getSelectedItem().equals("MM") || cbNgay.getSelectedItem().equals("DD") || cbKhoa.getSelectedItem().equals("") || cbGioiTinh.getSelectedItem().equals("") || cbLop.getSelectedItem().equals("")) {
                        ...
                        err.btnOK.addMouseListener(new MouseAdapter() {
                            public void mouseClicked(MouseEvent arg0) {
                                ...
                                String maLop = lopbus.getLopMaKhoa(maKhoa).get(cbLop.getSelectedIndex()).getMaLop();
                                String ngaySinh = cbNam.getSelectedItem().toString() + "-" + cbThang.getSelectedItem().toString() + "-" + cbNgay.getSelectedItem();
                                SinhVienDTO dtSV = new SinhVienDTO(maLop, maKhoa, txtHo.getText(), txtTen.getText(), ngaySinh, cbGioiTinh.getSelectedItem().toString(), 1);
                                svhoantac = sv.getObject(String.valueOf(model.getValueAt(i, 1)));
                                sv.sua(dtSV, String.valueOf(model.getValueAt(i, 1)));
        ...
        bts[3].addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent arg0) {
                if (check == 1) {
                    sv.khoiphuc(mahoantac);
                } else if (check == 2) {
                    sv.xoavinhvien(mahoantac);
                } else if (check == 3) {
                    sv.sua(svhoantac, mahoantac);
                } else if (check == 4) {
                    for (int i = 0; i < checkbox.size(); i++) {
                        sv.khoiphuc(checkbox.get(i));
                    }
                }

                vtData.clear();
                vtData.addAll(sv.getThongTin(Integer.parseInt(txtPage.getText())));
                model.setDataVector(vtData, vtHeader);
                tba.setModel(model);
                guiBase.isEnabled(bts[3]);
                check = 0;
        ...
        btImportExcel.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent arg0) {
                sv.importExcel(file.getFileExcelNameOpen(pnPaint));
            }
        });

        btExportExcel.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent arg0) {
                sv.exportExcel(file.getFileExcelNameSave(pnPaint));
            }
        });
